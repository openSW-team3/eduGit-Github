...
export class Skill extends LitElement {
  ...
  connectedCallback() {
    ...
    auth.addEventListener(
      AuthEvents.authStateChanged,
      this.authChanged.bind(this)
    );
    auth.addEventListener(
  ...
  render() {
    ...
    return html`
      <div
        ...
        @click="${this.toggleForceShowDescription}"
      ...
      >
        <div id="img-container">
          <img
            ...
            alt="${name}"
            role="presentation"
            data-src="${constructImagePathPrefix(collection, area, skill)}"
          />
        </div>
        <h6 id="title">${name}</h6>
      ...
      ${skills != null && !this.compact
        ? html`
            <div id="subskills" class="${skills.length > 1 ? "wide" : ""}">
              ${skills.length > 1
                ? html` <div class="arrow-connection"></div> `
                : undefined}
              ${repeat(
                skills,
                (skill) => getId(collection, area, skill),
                (skill) => html`
                  <div class="subskill">
                    <div class="arrow"></div>
                    <ws-skill
