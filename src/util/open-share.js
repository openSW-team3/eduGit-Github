...
export async function openShare({ title, text, url }) {
  ...
  const shareOptions = [
    ...
    [
      "Facebook",
      () =>
        openShareUrl(`https://www.facebook.com/sharer/sharer.php?u=${safeUrl}`),
    ],
    [
      "Twitter",
      () =>
        openShareUrl(
          `https://twitter.com/intent/tweet?&url=${safeUrl}&text=${safeText}`
        ),
    ],
    [
      "LinkedIn",
      () =>
        openShareUrl(
          `https://www.linkedin.com/shareArticle?mini=true&url=${safeUrl}&title=${safeTitle}&summary=${safeText}`
        ),
    ],
    [
      "Reddit",
      () =>
        openShareUrl(
          `https://www.reddit.com/submit?url=${safeUrl}&title=${safeText}`
        ),
    ],
    [
      "Email",
    ...
    [
      ...
      () =>
        openShareUrl(
          `https://news.ycombinator.com/submitlink?u=${safeUrl}&t=${safeText}`
        ),
    ],
    [
      "Blogger",
      () =>
        openShareUrl(
          `https://www.blogger.com/blog-this.g?n=${safeTitle}&b=${encodeURIComponent(
            `${text}. Find it <a href="${url}">here</a>.<br/><br/><img width="400" src="${img}" />`
          )}`
        ),
    ],
    [
      ...
      () =>
        openShareUrl(
          `https://www.tumblr.com/widgets/share/tool/preview?url=${safeUrl}`
        ),
    ],
    [
      "Pinterest",
      () =>
        openShareUrl(
          `https://www.pinterest.dk/pin/create/button/?url=${safeUrl}&description=${safeText}&media=${safeImg}`
        ),
    ],
    [
      "ВКонтакте",
      () =>
        openShareUrl(
          `https://vk.com/share.php?url=${safeUrl}&title=${safeTitle}&description=${safeText}&image=${safeImg}`
        ),
      "vk",
    ],
    [
      ...
      () =>
        openShareUrl(
          `https://connect.ok.ru/offer?url=${safeUrl}&title=${safeTitle}&imageUrl=${safeImg}`
        ),
      "ok",
    ],
