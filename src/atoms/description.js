...
import {
  measureCompleteSkill,
  measureShowDescription,
...
export class Description extends LitElement {
  ...
  connectedCallback() {
    ...
    auth.addEventListener(
      AuthEvents.completedSkillsChanged,
      this.requestUpdate
    );
  ...
  updateDirection() {
    ...
    this.directionX =
      left + width + directionOffset >= window.innerWidth ? "left" : "right";
    this.directionY =
      window.scrollY + top + height + directionOffset >=
  ...
  async toggleCompleteSkill() {
    ...
    if (newIsCompleted) {
      const { currentConfettiCount, sprayConfettiOnce, playAudio } =
        await import("./../util/confetti-helper.js");
    ...
    return html`
      <div class="link">
        <img
          ...
          height="16"
          intrinsicsize="16x16"
          src="https://plus.google.com/_/favicon?domain_url=${encodeURIComponent(
            getURLOrigin(url)
          )}"
          alt="Logo for ${name}"
        />
  ...
  render() {
    ...
    return html`
      ...
      ${description != null && description.text != null
        ? html`<p id="text">${description.text}</p>`
        : undefined}
      ${description != null &&
      description.links != null &&
      ...
      ${isAuthenticated
        ? html`
            <ws-button
              bordered
              id="complete-button"
              @click="${this.toggleCompleteSkill}"
